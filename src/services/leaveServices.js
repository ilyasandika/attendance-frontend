...
const leaveServices = {
    getLeaves: (page, search,  status, rows ) => api.get("/leaves", { params: { page, search, status, rows } }),
    getLeaveById: (id) => api.get(`/leaves/${id}`),
    getLeavesByUserId: (page, search, status, userId, rows) => api.get(`/leaves/user/${userId}`, { params: { page, search, status, rows } }),
